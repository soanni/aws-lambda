	...
	weather.getSmartJSON(function(err, smart){
		if(err){
			...
			if(Number(smart.temp) > 25){
				console.log("Temperature is greater than 25 degree Celcius!!");
				let snsParams = {
					Message: "It's Hot outside! WEATHER UPDATE: " + JSON.stringify(smart),
					Subject: 'WEATHER UPDATE',
					TopicArn: snsTopicARN
				};
				sns.publish(snsParams, function(snsErr, data){
					if(snsErr){
						console.log("An error occured while sending SNS Alert: " + snsErr, snsErr.stack);
						callback(snsErr);
					}else{
						console.log("SNS Alert sent successfully: ", snsParams.Message);
						callback(null,"Done");
					}
				});
			}else{
...
exports.handler = function(event, context, callback){
	...
	}else if(kmsEncryptedAPIKey && kmsEncryptedAPIKey !== '<API_key>'){
